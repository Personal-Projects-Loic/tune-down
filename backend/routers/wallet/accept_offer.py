...
async def accept_offer(
    ...
    if not request.sell_offer_id and not request.buy_offer_id:
        raise HTTPException(status_code=400, detail="Either sell_offer_id or buy_offer_id must be provided")
...
async def db_update_nft_owner(db: AsyncSession, nft_id: str, new_owner_id: str):
    try:
